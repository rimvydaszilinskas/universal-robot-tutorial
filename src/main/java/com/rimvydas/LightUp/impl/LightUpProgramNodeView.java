...
public class LightUpProgramNodeView implements SwingProgramNodeView<LightUpProgramNodeContribution> {
	...
	private Box createDurationSlider(final JSlider slider, int min, int max, final ContributionProvider<LightUpProgramNodeContribution> provider) { 
		Box box = Box.createHorizontalBox();
		box.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		final JLabel value = new JLabel(Integer.toString(slider.getValue()) + " seconds");
	...
	private Box createIOComboBox(final JComboBox<Integer> combo, final ContributionProvider<LightUpProgramNodeContribution> provider) {
		Box box = Box.createHorizontalBox();
		box.setAlignmentX(Component.LEFT_ALIGNMENT);
